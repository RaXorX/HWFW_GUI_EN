    ...
    case 0:
    {
      ...
      case IDM_OPEN:
      {
        ...
        if (GetOpenFilePath(hDlg, wsPath, MAX_PATH))
        {
          ...
          SetStatus(L"Failed to obtain file path!");
      }
      break;
      ...
      case IDM_SAVEAS:
      {
        ...
        if (GetSaveFilePath(hDlg, wsTmp, MAX_PATH))
        {
          ...
          if (nResult == 0)
            SetStatus(L"File saving complete.");
          else
          {
            swprintf_s(wsTmp, L"File save failed, error code: [%d]!", nResult);
            SetStatus(wsTmp);
      ...
      case IDM_CHKCRC:
      {
        ...
        if (u32Result == CHKCRC_OK)
          SetStatus(L"CRC32 OK.");
        else if (u32Result == CHKCRC_HEADERCRCERR)
          SetStatus(L"Header CRC32 BAD!");
        else if (u32Result == CHKCRC_FILECRCERR)
          SetStatus(L"Data CRC32 BAD!");
        else if (u32Result >= CHKCRC_ITEMCRCERR)
        {
          WCHAR wsTmp[128];

          swprintf_s(wsTmp, L"Module verification failed, Index: [%u]!", u32Result - CHKCRC_ITEMCRCERR);
          SetStatus(wsTmp);
        }
        else
