...
INT_PTR CALLBACK DlgProc_ItemInfo(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_INITDIALOG:
  {
    ...
    if (lpDlgParam->dtType == DT_EDIT)
    {
      ...
      if (nResult != 0)
      {
        swprintf_s(wsTemp, L"Failed to obtain project information! Error code: [%d]", nResult);
    ...
    if (lParam != 0)
    {
      ...
      case IDC_BTN_EXP:
        if (lpDlgParam->dtType == DT_EDIT)
        {
          ...
          if (GetSaveFilePath(hDlg, wsTmp, MAX_PATH))
          {
            ...
            if (nResult != 0)
            {
              SetTooltip(GetDlgItem(hDlg, IDC_LBL_II_STATUS), L"Failed to obtain the project data size, error code:[%d]!", nResult);
              break;
            }
            ...
            if (nResult != 0)
            {
              SetTooltip(GetDlgItem(hDlg, IDC_LBL_II_STATUS), L"Failed to obtain project data, error code:[%d]!", nResult);
              break;
            }
            ...
            if (ExportToFile(wsTmp, lpData, u32DataSize))
              ...
              SetTooltip(GetDlgItem(hDlg, IDC_LBL_II_STATUS), L"Failed to export project data, error code: [%d]!", GetLastError());
          }
        }
        break;
      ...
      case IDC_BTN_IMP:
        if (lpDlgParam->dtType == DT_EDIT)
        {
          ...
          if (GetOpenFilePath(hDlg, wsTmp, MAX_PATH))
          {
            ...
            if (nResult != 0)
              SetTooltip(GetDlgItem(hDlg, IDC_LBL_II_STATUS), L"Failed to import file, error code: [%d]!", nResult);
            else
              SetTooltip(GetDlgItem(hDlg, IDC_LBL_II_STATUS), L"File import completed.");
      ...
      case IDOK:
      {
        ...
        else if (lpDlgParam->dtType == DT_ADD)
        {
          ...
          {
            SetTooltip(GetDlgItem(hDlg, IDC_LBL_II_STATUS), L"Failed to import file!");
            break;
          }
        ...
        {
          SetTooltip(GetDlgItem(hDlg, IDC_LBL_II_STATUS), L"Failed to save file, error code: [%d]!", nResult);
        }
      }
      break;
